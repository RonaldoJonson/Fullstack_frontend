...
function VideoPage({
    ...
    const NextVideoMiniature = () => {
        return <div id={'nextBar'} style={{paddingBottom:'10px'}} onDrop={Drop} onDragOver={DragOver}>
        {nextVideos.map(currentVideo => {
            return <M_Video_CompactMiniature key={currentVideo.title} video={currentVideo}/>
        })}
        </div>
    }

    const SideBarVideoMiniatures = () => {
        return <div id={'sideBar'} style={{paddingBottom:'10px'}} onDrop={Drop} onDragOver={DragOver}>
        {sideVideos.map(currentVideo => {
            return <M_Video_CompactMiniature key={currentVideo.title} video={currentVideo}/>
        })}
        </div>
    }
